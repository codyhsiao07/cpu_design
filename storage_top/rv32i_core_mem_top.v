// rv32i_core_mem_top.v -- 5-stage RV32I core with integrated MEM/cache subsystem
...
module rv32i_core_mem_top #(
  ...
  wire        mem_addr_misaligned = mem_valid &&
                                    ...
                                    (
                                      (mem_size[1:0] == 2'b10 && mem_alu_result[1:0] != 2'b00) ||
  ...
  u_mem_top (
    ...
    .mem_err_event_o  (mem_err_event),
    .ifetch_valid_i   (~stall_if),
    .ifetch_addr_i    (if_pc),
  ...
  wire wb_i_valid = mem_mem_read ?
                    (mem_load_valid & ~mem_err_event) :
                    (mem_valid & ~mem_stall);
  ...
  mem_wb u_mem_wb (
    ...
    .i_stall      (1'b0),
    .i_rd         (mem_rd),
    .i_rd_wen     (mem_reg_write & ~mem_err_event),
