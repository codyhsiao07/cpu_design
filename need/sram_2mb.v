...
// -----------------------------------------------------------------------------
module sram_2mb #(
  parameter [31:0] SRAM_BASE_ADDR  = 32'h8000_0000,
  parameter [31:0] SRAM_SIZE_BYTES = 32'd2097152,   // 2 MiB
  parameter [31:0] SRAM_LAST_ADDR  = 32'h801F_FFFF, // base + size - 1

  // Region partitions (must fully cover SRAM window)
  parameter [31:0] TEXT_BASE_ADDR  = 32'h8000_0000,
  parameter [31:0] TEXT_LAST_ADDR  = 32'h800F_FFFF,
  parameter [31:0] DATA_BASE_ADDR  = 32'h8010_0000,
  parameter [31:0] DATA_LAST_ADDR  = 32'h801B_FFFF,
  parameter [31:0] STACK_BASE_ADDR = 32'h801C_0000,
  parameter [31:0] STACK_LAST_ADDR = 32'h801F_FFFF,
...
`ifndef SYNTHESIS
  task automatic tb_poke;
    input [31:0] addr;
    input [31:0] data;
    integer idx;
    begin
      if (addr < SRAM_BASE_ADDR || addr > SRAM_LAST_ADDR) begin
        $display("%t [SRAM] TB_POKE WARN: address 0x%08h out of range.", $time, addr);
      end else begin
        idx = (addr - SRAM_BASE_ADDR) >> 2;
        mem[idx] = data;
      end
    end
  endtask

  function [31:0] tb_peek;
    input [31:0] addr;
    integer idx;
    begin
      if (addr < SRAM_BASE_ADDR || addr > SRAM_LAST_ADDR) begin
        $display("%t [SRAM] TB_PEEK WARN: address 0x%08h out of range.", $time, addr);
        tb_peek = 32'h0;
      end else begin
        idx = (addr - SRAM_BASE_ADDR) >> 2;
        tb_peek = mem[idx];
